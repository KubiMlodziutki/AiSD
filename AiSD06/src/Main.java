        ...
        while(!halt) {
            ...
            if(word[0].equalsIgnoreCase("add") && word.length==2) {
                ...
                for (int i = 0; i < word[1].length(); i++){
                    if (word[1].charAt(i) == '('){
                        if (word[1].charAt(i + 1) == ')' || i == word[1].length() - 1){
                            link = Document.createLink(word[1]);
                            break;
                        }
                        counting = true;
                        continue;
                    ...
                    if (counting){
                        if (word[1].charAt(i) == ')'){
                            ...
                            for (int j = 0; j < word[1].length(); j++){
                                ...
                                wordToAdd += word[1].charAt(j);
                            }
                            link = Document.createLink(wordToAdd,Integer.parseInt(weight));
                            break;
                        }
                        if (i == word[1].length() - 1){
                            link = Document.createLink(word[1]);
                        }
                        if (Character.isDigit(word[1].charAt(i))){
                            weight += word[1].charAt(i);
                        }
                        else{
                            link = Document.createLink(word[1]);
                        }
